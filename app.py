...
def load_data():
    ...
    # Решение проблемы №1: Приравниваем "Сляб" и "СЛЯБ"
...
try:
    ...
    if not selected_months:
        st.warning("Выберите месяц.")
        st.stop()

    filtered_df = df[df['Месяц_Фи|льтр'].isin(selected_months)]
    ...
    st.divider()
    st.subheader("Распределение по сортаментам (масса в тн)")
    ...
    # Применяем форматирование: закрашиваем ячейки в зависимости от значения массы
    styled_matrix = matrix_table.style.background_gradient(cmap='Greens') \
                                      .format("{:.1f}") \
